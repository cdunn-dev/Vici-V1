...
export async function registerRoutes(app: Express) {
  ...
  app.get("/api/strava/debug", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "Not authenticated" });
      }
      ...
      const redirectUri = `https://${domain}/api/auth/strava/callback`;
      
      // Check if environment variables are set (mask actual values)
      const clientIdSet = !!process.env.VITE_STRAVA_CLIENT_ID;
      const clientSecretSet = !!process.env.STRAVA_CLIENT_SECRET;
      
      // Get user Strava connection status
      const stravaConnected = req.user?.connectedApps?.includes("strava");
      const hasStravaTokens = !!req.user?.stravaTokens;
      
      let tokenStatus = null;
      if (hasStravaTokens) {
        // Check if tokens are expired
        const now = Math.floor(Date.now() / 1000);
        const expiresAt = req.user?.stravaTokens?.expiresAt;
        tokenStatus = {
          isExpired: expiresAt < now,
          expiresIn: expiresAt - now,
          hasRefreshToken: !!req.user?.stravaTokens?.refreshToken
        };
      }
      ...
      res.json({
        environment: {
          domain,
          redirectUri,
          clientIdConfigured: clientIdSet,
          clientSecretConfigured: clientSecretSet,
      ...
      console.error("Error in Strava debug endpoint:", error);
